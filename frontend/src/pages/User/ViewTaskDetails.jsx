    ...
    if (todoChecklist && todoChecklist[index]) {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          //optionally revert toggle if the API call fails
          todoChecklist[index].completed = !todoChecklist[index].completed;
        }
      } catch (error) {
        todoChecklist[index].completed = !todoChecklist[index].completed;
...
const InfoBox = ({ label, value }) => {
  return (
    <>
      ...
      <p className="text-[12px] md:text-[13px] font-medium text-gray-700 mt-1">
        {value}
      </p>
    </>
  );
...
const Attachment = ({ link, index, onClick }) => {
  return (
    ...
    >
      ...
      <LuSquareArrowOutUpRight className="text-lg text-gray-500" />
    </div>
  );
};
